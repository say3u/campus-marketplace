  ...
  return (
    ...
    <nav className="border-b"
      ...
      <div className="h-16 flex items-center gap-3 px-4 sm:px-6">
        ...
        <form onSubmit={handleSearch} className="hidden sm:block flex-1 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 ml-auto">
          ...
          <Link to={user ? '/listings/new' : '/register'}
            className="text-sm font-bold text-white px-3 sm:px-5 py-2 rounded-lg transition-opacity hover:opacity-85 flex-shrink-0 whitespace-nowrap"
          ...
          {user ? (
            <>
              <Link to="/profile" className="hidden sm:block text-sm font-medium transition-colors"
                style={{ color: dark ? '#D4D4D4' : '#374151' }}>{user.username}</Link>
              <button onClick={handleLogout} className="hidden sm:block text-sm transition-colors"
            ...
            <>
              ...
              <Link to="/register" className="text-sm font-medium transition-colors flex-shrink-0"
      ...
      {/* Mobile nav links */}
      {user && (
        <div className="md:hidden flex items-center gap-5 px-4 pb-2.5 text-sm font-medium overflow-x-auto">
          <Link to="/browse" style={{ color: dark ? '#A3A3A3' : '#4B5563' }}>Browse</Link>
          <Link to="/favorites" style={{ color: dark ? '#A3A3A3' : '#4B5563' }}>Saved</Link>
          <Link to="/messages" className="relative" style={{ color: dark ? '#A3A3A3' : '#4B5563' }}>
            Messages
            {totalUnread > 0 && (
              <span className="absolute -top-1.5 -right-3 min-w-[16px] h-4 flex items-center justify-center rounded-full text-white px-1"
                style={{ backgroundColor: '#f43f5e', fontSize: '10px', fontWeight: 700 }}>
          ...
          <Link to="/profile" style={{ color: dark ? '#A3A3A3' : '#4B5563' }}>{user.username}</Link>
          <button onClick={handleLogout} style={{ color: dark ? '#737373' : '#9CA3AF' }}>Sign out</button>
        </div>
      )}
    </nav>
    </div>
