...
export default function Home() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6" style={{ backgroundColor: '#0f172a' }}>
      {/* Search row */}
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-md pl-9 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40"
        ...
        <button
          ...
          style={{
            backgroundColor: showFilters || hasFilters ? 'rgba(79,70,229,0.1)' : '#1e293b',
            borderColor: showFilters || hasFilters ? '#3B82F6' : '#475569',
            color: showFilters || hasFilters ? '#3B82F6' : '#94a3b8',
          }}
        >
      ...
      {/* Category tabs */}
      <div className="flex gap-0 flex-wrap mb-6 border-b" style={{ borderColor: '#475569' }}>
        {CATEGORIES.map(cat => {
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {Array.from({ length: 10 }).map((_, i) => (
            <div key={i} className="rounded-xl border animate-pulse" style={{ backgroundColor: '#1e293b', borderColor: '#475569' }}>
              <div className="aspect-square rounded-t-xl" style={{ backgroundColor: '#1e3a5f' }} />
              <div className="p-3 space-y-2">
                <div className="h-3 rounded w-3/4" style={{ backgroundColor: '#1e3a5f' }} />
                <div className="h-3 rounded w-1/2" style={{ backgroundColor: '#1e3a5f' }} />
              </div>
            </div>
          ))}
        </div>
      ) : listings.length === 0 ? (
        <div className="text-center py-20 rounded-xl border" style={{ borderColor: '#475569', borderStyle: 'dashed' }}>
