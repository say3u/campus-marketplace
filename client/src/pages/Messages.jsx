...
function ChatPanel({ conversationId, other }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      {/* Chat header */}
      <div className="px-5 py-3.5 border-b flex items-center gap-3" style={{ borderColor: '#475569' }}>
            ...
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className="max-w-xs lg:max-w-sm">
                <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                  ...
                  backgroundColor: mine ? '#3B82F6' : '#1e3a5f',
                  color: mine ? '#fff' : '#e2e8f0',
                }}>
                  {m.body}
                </div>
                <p className="text-xs mt-1 px-1" style={{ color: '#475569', textAlign: mine ? 'right' : 'left' }}>
      ...
      {/* Input */}
      <form onSubmit={handleSend} className="p-4 border-t flex gap-2" style={{ borderColor: '#475569' }}>
        <input
          value={draft}
...
export default function Messages() {
  ...
  return (
    <div className="max-w-4xl mx-auto flex border-x" style={{ borderColor: '#475569', height: 'calc(100vh - 56px)' }}>

      {/* Sidebar */}
      <div className="w-64 flex flex-col border-r flex-shrink-0" style={{ borderColor: '#475569', backgroundColor: '#0f172a' }}>
        <div className="px-4 py-4 border-b" style={{ borderColor: '#475569' }}>
          <h2 className="text-sm font-semibold text-white">Messages</h2>
        </div>
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center">
              <MessageSquare size={28} style={{ color: '#475569' }} className="mb-2" />
              ...
              <Link key={c.id} to={`/messages/${c.id}`}
                ...
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                    style={{ backgroundColor: isActive ? '#3B82F6' : '#1e3a5f' }}>
                    {other[0].toUpperCase()}
                  </div>
      ...
      {/* Chat */}
      <div className="flex-1 flex flex-col min-h-0" style={{ backgroundColor: '#0f172a' }}>
        {activeId
